		...
		<div style="float:left; width:35%; text-align:left;">
			<strong>
				<div style="font-size:<?php echo $title_size;?>px;  margin-top:15px;">
					...
					_e('Order: '.$order->get_order_number(), 'wf-woocommerce-packing-list');
					?>
				</div>
			</strong>
			<div style="margin-top:10px; font-size:<?php echo $title_size;?>px;"><?php
			_e(('Order Date: '.date("d-m-Y", strtotime((WC()->version < '2.7.0') ? $order->order_date : $order->get_date_created()))) , 'wf-woocommerce-packing-list'); ?>
			</div>
		</div>
