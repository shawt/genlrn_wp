				...
				<li id="slider-posts-tabs-content-0"  class="active">
					...
					<ul id="huge-it-posts-list" class="list">
						...
						if(isset($rowspostspop[0]->ID)) {
							...
							if(isset($rowspostspop[0]->ID)) {							if(get_the_post_thumbnail($rowspostspop[0]->ID, 'thumbnail') != ''){
								$strx++;
								$hascolor="";
					...
					<form method="post"  onkeypress="doNothing()" action="admin.php?page=sliders_huge_it_slider&task=popup_posts&id=<?php echo $_GET['id']; ?>&closepop=1" id="huge-it-category-form" name="admin_form">
								<div>
									<div class="left less-margin height">
										...
										<?php foreach ($categories as $strcategories){ ?>
											<option <?php if(isset($rowimages->name) && $rowimages->name == $strcategories->cat_name){echo 'selected="selected"';} ?> value="<?php echo $strcategories->cat_name; ?>"><?php echo $strcategories->cat_name; ?></option>
										<?php	}	?> 
										</select>
									</div>
...
?>
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {			
			...
			jQuery('#huge_it_add_video_input').change(function(){
				...
				if (jQuery(this).val().indexOf("youtube") >= 0 || jQuery(this).val().indexOf("youtu") >= 0){
	...
	<div id="huge_it_slider_add_videos">
		<div id="huge_it_slider_add_videos_wrap">
			...
			<div class="control-panel">
				<form method="post"  onkeypress="doNothing()" action="admin.php?page=sliders_huge_it_slider&task=popup_video&id=<?php echo $_GET['id']; ?>&closepop=1" id="huge-it-video-form" name="admin_form">
